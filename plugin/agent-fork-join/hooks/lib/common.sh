...
# Add a comment to a beads issue AND post directly to JIRA
# NOTE: Beads comments do NOT sync to JIRA via bd jira sync --push
#       This function posts comments directly to JIRA via REST API
...
beads_add_comment() {
	...
	bd comments add "$issue_id" "$comment" 2>/dev/null || {
		log_debug "Could not add comment to beads issue $issue_id"
		return 1
	}

	# Post comment directly to JIRA (beads comments don't sync to JIRA)
	local external_ref jira_key
	# bd show --json returns an array, so we need .[0]
	external_ref=$(bd show "$issue_id" --json 2>/dev/null | jq -r '.[0].external_ref // empty' 2>/dev/null || echo "")

	if [[ -n "$external_ref" ]]; then
		# Extract JIRA key from URL (e.g., https://badal.atlassian.net/browse/PGF-123 -> PGF-123)
		jira_key=$(echo "$external_ref" | grep -oE '[A-Z]+-[0-9]+' | tail -1 || echo "")

		if [[ -n "$jira_key" ]]; then
			# Find the jira-comment.sh script
			# Try multiple paths to locate it
			local jira_script=""
			local repo_root
			repo_root="$(git rev-parse --show-toplevel 2>/dev/null || echo "")"

			if [[ -n "$repo_root" && -x "${repo_root}/plugins/jira/scripts/jira-comment.sh" ]]; then
				jira_script="${repo_root}/plugins/jira/scripts/jira-comment.sh"
			elif [[ -n "${CLAUDE_PLUGIN_ROOT:-}" && -x "${CLAUDE_PLUGIN_ROOT}/../jira/scripts/jira-comment.sh" ]]; then
				jira_script="${CLAUDE_PLUGIN_ROOT}/../jira/scripts/jira-comment.sh"
			fi

			if [[ -n "$jira_script" && -x "$jira_script" ]]; then
				"$jira_script" "$jira_key" "$comment" 2>/dev/null || {
					log_debug "Could not post comment to JIRA $jira_key"
					# Don't fail - local comment was added successfully
				}
			else
				log_debug "JIRA comment script not found at $jira_script"
			fi
		else
			log_debug "Could not extract JIRA key from external_ref: $external_ref"
		fi
	else
		log_debug "No external_ref found for issue $issue_id, skipping JIRA comment"
	fi
}
