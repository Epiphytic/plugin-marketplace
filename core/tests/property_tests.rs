...
use gear_core::aisp::{AispConverter, ConversionTier};
...
const VERBS: &[&str] = &[
    "must", "should", "requires", "ensures", "provides", "returns", "implies", "contains",
    "equals", "is", "defined as", "maps to", "calls", "validates", "checks",
];

const OBJECTS: &[&str] = &[
    "access", "token", "valid", "true", "false", "null", "empty", "unique", "secure", "authenticated",
    "5", "10", "data", "list", "set", "graph",
];

const MODIFIERS: &[&str] = &[
    "for all", "there exists", "if", "then", "else", "and", "or", "not", "always", "never",
];
...
fn test_fuzz_conversion() {
    ...
    for _i in 0..100 {
        ...
        if result.tier == ConversionTier::Full {
            assert!(result.output.contains("⟦Ω:Meta⟧"), "Missing Meta block in Full tier for: {}", prose);
            assert!(result.output.contains("⟦Σ:Types⟧"), "Missing Types block in Full tier for: {}", prose);
            assert!(result.output.contains("⟦Γ:Rules⟧"), "Missing Rules block in Full tier for: {}", prose);
            assert!(result.output.contains("⟦Ε⟧"), "Missing Evidence block in Full tier for: {}", prose);
        }
...
fn test_detect_tier_correctness() {
    ...
    for (prose, expected) in cases {
        ...
        if expected == ConversionTier::Full {
            assert_eq!(detected, ConversionTier::Full, "Failed to detect Full tier for: {}", prose);
        } else if expected == ConversionTier::Standard {
             assert!(detected == ConversionTier::Standard || detected == ConversionTier::Full, 
                "Expected Standard or higher for: {}, got {}", prose, detected);
        }
    }
}
...
fn test_error_inference() {
    ...
    let result = AispConverter::convert(prose, Some(gear_core::ConversionOptions {
        tier: Some(ConversionTier::Full),
        ..Default::default()
    }));
    
    assert!(result.output.contains("⟦Χ:Errors⟧"), "Missing Errors block");
    assert!(result.output.contains("fail(x)⇒⊥"), "Missing fail inference");
    assert!(result.output.contains("crash⇒⊥⊥"), "Missing crash inference");
    assert!(result.output.contains("NotFound⇒∅"), "Missing NotFound inference");
}
...
fn test_contractor_inference() {
    ...
    assert!(result.output.contains("Inv(s)"), "Missing Invariant inference");
    assert!(result.output.contains("Pre(f)"), "Missing Precondition inference");
    assert!(result.output.contains("Post(f)"), "Missing Postcondition inference");
